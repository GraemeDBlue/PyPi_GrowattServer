...
for plant in plant_list["data"]:
    ...
    if is_noah["result"] == 1 and (
        ...
        print()  # noqa: T201
        indent_print(
            "Remaining battery (" + "%" + "): {}".format(noah_system["obj"]["soc"]), 2
        )
        indent_print("Solar Power (w): {}".format(noah_system["obj"]["ppv"]), 2)
        indent_print(
            "Charge Power (w): {}".format(noah_system["obj"]["chargePower"]), 2
        )
        indent_print(
            "Discharge Power (w): {}".format(noah_system["obj"]["disChargePower"]), 2
        )
