...
def main():
    ...
    try:
        ...
        print(f"Found {plants['count']} plants")
        ...
        if not sph_mix_device:
            print("No SPH/MIX device found!")
            return

        device_sn = sph_mix_device['device_sn']
        print(f"Found SPH/MIX device: {device_sn}")

        # Example 1: Set AC charge time period with high power charging
        print("\n=== Setting AC Charge Time Period ===")
        ...
        print(f"AC charge time period set successfully: {result}")

        # Example 2: Alternative approach using write_parameter method
        print("\n=== Alternative: Using write_parameter method ===")
        ...
        print(f"Alternative charge time period set: {result2}")

        # Example 3: Disable charging during a specific period
        print("\n=== Disabling Charge Time Period ===")
        ...
        print(f"Charge time period disabled: {result3}")
        ...
        print("Current charge time settings:")
        for i in range(1, 4):  # Typically 3 time periods
            ...
            if start_key in settings:
                print(f"  Period {i}:")
                print(f"    Start: {settings.get(start_key, 'N/A')}")
                print(f"    Stop: {settings.get(stop_key, 'N/A')}")
                print(f"    Enabled: {settings.get(enabled_key, 'N/A')}")
...
def demonstrate_other_parameters():
    ...
    print(f"Backflow params: {backflow_params}")
    
    # PV on/off control
    pv_params = growattServer.OpenApiV1.PvOnOffParams(
        pv_enabled=True  # Turn PV on
    )
    print(f"PV control params: {pv_params}")
    ...
    print(f"Voltage params: {voltage_params}")
    ...
    print(f"Off-grid params: {offgrid_params}")
